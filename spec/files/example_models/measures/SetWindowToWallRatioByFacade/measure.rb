...
class SetWindowToWallRatioByFacade < OpenStudio::Ruleset::ModelUserScript
  ...
  def run(model, runner, user_arguments)
    ...
    surfaces.each do |s|
      ...
      if facade == 'North'
        next unless azimuth >= 315.0 or azimuth < 45.0
      elsif facade == 'East'
        next unless azimuth >= 45.0 and azimuth < 135.0
      elsif facade == 'South'
        next unless azimuth >= 135.0 and azimuth < 225.0
      elsif facade == 'West'
        next unless azimuth >= 225.0 and azimuth < 315.0
      else
