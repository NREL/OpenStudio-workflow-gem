...
# start the measure
class PurposefullyBrokenReportingMeasure < OpenStudio::Measure::ReportingMeasure
  # human readable name
  def name
    # Measure name should be the title case of the class name.
    return 'Purposefully Broken Reporting Measure'
  end

  # human readable description
  def description
    return 'A reporting measure that will fail in the reporting phase (after model successfully ran in E+) to test for #864'
  end
  ...
  def run(runner, user_arguments)
    ...
    raise "I'm purposefully breaking the reporting! Before the report was already created!"
...
PurposefullyBrokenReportingMeasure.new.registerWithApplication
